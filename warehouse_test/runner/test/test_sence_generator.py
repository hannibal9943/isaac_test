    ...
    def generate_scene(self, grid: Grid, world: World, save_path: str):
        ...
        for i in range(grid.rows):
            for j in range(grid.cols):
                ...
                output_tile['grid_rows'].append(i)
                output_tile['grid_cols'].append(j)
                output_tile['tile_name'].append(tile_name)
        ...
        # Save the tile
        save_path_lastpose = save_path.rfind('/')
        tile_path = os.path.join(f"{save_path[0:save_path_lastpose]}/", f"tile.pickle")
        with open(tile_path, 'wb') as fw:
            pickle.dump(output_tile, fw)
